...
class CMCMem_mix(BaseMem):
    """Memory bank for two modalities, e.g. in CMC"""
    def __init__(self, n_dim, n_data, K=65536, T=0.07, m=0.5):
        super(CMCMem_mix, self).__init__(K, T, m)
        # create sampler
        self.multinomial = AliasMethod(torch.ones(n_data))
        self.multinomial.cuda()
    ...
    def forward(self, x1, x2, y, x1_jig=None, x2_jig=None,
        ...
        mask = np.random.beta(self.alpha, self.alpha, size=w1.shape)
        mask = torch.from_numpy(mask).float().cuda()

        mixed_w1 = mask * w1 + (1 - mask) * w2
        mixed_w2 = mask * w2 + (1 - mask) * w1

        # compute logits
        logits1 = self._compute_logit(x1, mixed_w2)
        logits2 = self._compute_logit(x2, mixed_w1)
        if (x1_jig is not None) and (x2_jig is not None):
            logits1_jig = self._compute_logit(x1_jig, mixed_w2)
            logits2_jig = self._compute_logit(x2_jig, mixed_w1)
